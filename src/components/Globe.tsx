...
export const Globe: React.FC<GlobeProps> = ({ theme, scale = 1.2 }) => {
  ...
  const themeRef = useRef(theme); // ← track theme without triggering re-init
  const [isReady, setIsReady] = useState(false);

  // ---- Sync themeRef when prop changes ----
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const globeConfig: COBEOptions = {
      ...
      onRender: (state) => {
        ...
        // ---- Read theme from ref — no re-init needed ----
        ...
        if (sizeRef.current > 0) {
          globeState.width = sizeRef.current * 2;
          globeState.height = sizeRef.current * 2;
        }
        ...
        const rippleConfigs: { maxScale: number }[] = [
          { maxScale: 24 },
          { maxScale: 10 }
        ];
